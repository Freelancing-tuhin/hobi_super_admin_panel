...
const TotalIncome = ({ basicData }: any) => {
  const [transformedData, setTransformedData] = useState<number[]>([]);

  useEffect(() => {
    if (basicData?.incomeByMonth && Array.isArray(basicData.incomeByMonth)) {
      const updatedData = basicData.incomeByMonth.map((value: number) =>
        value === 0 ? 50 : value,
      );
    ...
    series: [
      {
        ...
        data: transformedData.length > 0 ? transformedData : Array(12).fill(50), // Fallback to [50,50,50,...]
      },
    ],
    stroke: {
      curve: 'smooth',
    ...
    <CardBox className="mt-30">
      ...
      <div className="grid grid-cols-12 gap-6 mt-4">
        <div className="col-span-6">
          <h4 className="text-2xl pb-1">₹{basicData?.totalIncome}</h4>
