...
const RevenueForcast = ({ basicData }: any) => {
  const [chartSeries, setChartSeries] = useState([
    { name: 'Events', data: Array(8).fill(0) },
    { name: 'Income', data: Array(8).fill(0) },
    { name: 'Customers', data: Array(8).fill(0) },
  ]);

  useEffect(() => {
    if (basicData) {
      setChartSeries([
        { name: 'Events', data: basicData.eventsByMonth || Array(8).fill(0) },
        { name: 'Bookings', data: basicData.bookingByMonth || Array(8).fill(0) },
        { name: 'Customers', data: basicData.userByMonth || Array(8).fill(0) },
      ]);
    }
  }, [basicData]);
  ...
  const ChartData: any = {
    ...
    markers: {
      strokeColor: ['var(--color-error)', 'var(--color-secondary)', 'var(--color-primary)'],
      strokeWidth: 2,
    },
    tooltip: { theme: 'dark' },
  };
      ...
      <div className="md:flex justify-between items-center">
        <div className="flex gap-4 items-center">
          <span className="h-12 w-12 flex-shrink-0 flex items-center justify-center bg-lightprimary rounded-tw">
            <Icon icon="solar:layers-linear" className="text-primary" height={24} />
          </span>
          <div>
